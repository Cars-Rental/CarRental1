...
import { FEATURED_RENTAL_CARS, FEATURED_BUY_CARS } from "../constants/landing.constants";
...
export function FeaturedCars({ locale, mode }: FeaturedCarsProps) {
    ...
    <section className="py-20 bg-white dark:bg-slate-900" dir={isRTL ? "rtl" : "ltr"}>
      <div className="container-main">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {cars.map((car) => (
            <CarCard key={car.id} car={car} locale={locale} mode={mode} />
          ))}
        </div>
      </div>
    </section>
